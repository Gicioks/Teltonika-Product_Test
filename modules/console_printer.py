...
class ConsolePrinter:
    ...
    def update_test_output(self, command, args):
        self.__current += 1
        self.__update_top()
        self.__update_case(command, args)
    ...
    def __update_case(self, command = "N/A", args = None):
        if args:
            line = "Command: " + command + " " + str(args)
        else:
            line = "Command: " + command
        self.__console_print_list[1] = line
    def __update_status(self, status=None, command="N/A"):
        line = "Command '" + command + "' status: "
        if status is not None and status is True:
